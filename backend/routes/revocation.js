...
router.get("/confirmed-needs-replacement", authenticate, requireRoles("ADMIN"), adminLimiter, async (req, res) => {
  const ip = req.ip || "";
  const ua = req.get("user-agent") || "";
  try {
    const items = await revocationService.listConfirmedNeedsReplacement();
    await auditService.log({
      id_agent: req.agent._id,
      action: auditService.ACTIONS.LIST_REVOCATIONS,
      ip_address: ip,
      user_agent: ua,
      resultat: "SUCCESS",
      details: "confirmed-needs-replacement",
    });
    return res.json({ items });
  } catch (e) {
    await auditService.log({
